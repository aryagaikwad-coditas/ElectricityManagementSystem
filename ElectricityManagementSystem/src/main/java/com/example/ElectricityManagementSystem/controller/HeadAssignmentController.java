...
public class HeadAssignmentController {
    ...
    @PostMapping("/state-head")
    @PreAuthorize("hasRole('MANAGEMENT')")
    public ResponseEntity<String> assignStateHead(
            @RequestBody AssignStateHeadRequest req) {
        headAssignmentService.assignStateHead(req);
        return ResponseEntity.ok("State Head assigned successfully");
    }

    @GetMapping("/state-head/by-state/{stateId}")
    public ResponseEntity<?> getStateHeadsByState(@PathVariable Long stateId) {
        return ResponseEntity.ok(
            headAssignmentService.getStateHeadsByState(stateId));
    }

    @GetMapping("/state-head/by-user/{userId}")
    public ResponseEntity<?> getStatesByStateHead(@PathVariable Long userId) {
        return ResponseEntity.ok(
            headAssignmentService.getStatesByStateHead(userId));
    }

    @DeleteMapping("/state-head")
    @PreAuthorize("hasRole('MANAGEMENT')")
    public ResponseEntity<String> removeStateHead(
            @RequestParam Long userId,
            @RequestParam Long stateId) {
        headAssignmentService.removeStateHead(userId, stateId);
        return ResponseEntity.ok("State Head removed");
    }
    ...
    @PostMapping("/district-head")
    @PreAuthorize("hasAnyRole('MANAGEMENT','STATE_HEAD')")
    public ResponseEntity<String> assignDistrictHead(
            @RequestBody AssignDistrictHeadRequest req) {
        headAssignmentService.assignDistrictHead(req);
        return ResponseEntity.ok("District Head assigned successfully");
    }

    @GetMapping("/district-head/by-district/{districtId}")
    public ResponseEntity<?> getDistrictHeadsByDistrict(
            @PathVariable Long districtId) {
        return ResponseEntity.ok(
            headAssignmentService.getDistrictHeadsByDistrict(districtId));
    }

    @GetMapping("/district-head/by-user/{userId}")
    public ResponseEntity<?> getDistrictsByDistrictHead(
            @PathVariable Long userId) {
        return ResponseEntity.ok(
            headAssignmentService.getDistrictsByDistrictHead(userId));
    }

    @DeleteMapping("/district-head")
    @PreAuthorize("hasAnyRole('MANAGEMENT','STATE_HEAD')")
    public ResponseEntity<String> removeDistrictHead(
            @RequestParam Long userId,
            @RequestParam Long districtId) {
        headAssignmentService.removeDistrictHead(userId, districtId);
        return ResponseEntity.ok("District Head removed");
    }
    ...
    @PostMapping("/city-head")
    @PreAuthorize("hasAnyRole('MANAGEMENT','STATE_HEAD','DISTRICT_HEAD')")
    public ResponseEntity<String> assignCityHead(
            @RequestBody AssignCityHeadRequest req) {
        headAssignmentService.assignCityHead(req);
        return ResponseEntity.ok("City Head assigned successfully");
    }
    ...
    @PostMapping("/technician-area")
    @PreAuthorize("hasAnyRole('CITY_HEAD','DISTRICT_HEAD')")
    public ResponseEntity<String> assignTechnicianToArea(
            @RequestBody AssignTechnicianToAreaRequest req) {
        headAssignmentService.assignTechnicianToArea(req);
        return ResponseEntity.ok("Technician assigned to area");
    }

    @GetMapping("/technician-area/by-area/{areaId}")
    public ResponseEntity<?> getTechniciansByArea(@PathVariable Long areaId) {
        return ResponseEntity.ok(
            headAssignmentService.getTechniciansByArea(areaId));
    }
    ...
    @PostMapping("/biller-city")
    @PreAuthorize("hasAnyRole('CITY_HEAD','DISTRICT_HEAD')")
    public ResponseEntity<String> assignBillerToCity(
            @RequestBody AssignBillerToCityRequest req) {
        headAssignmentService.assignBillerToCity(req);
        return ResponseEntity.ok("Biller assigned to city");
    }

    @GetMapping("/biller-city/by-city/{cityId}")
    public ResponseEntity<?> getBillersByCity(@PathVariable Long cityId) {
        return ResponseEntity.ok(
            headAssignmentService.getBillersByCity(cityId));
    }
}
